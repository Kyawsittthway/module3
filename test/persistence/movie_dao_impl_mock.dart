...
import 'package:module3/persistence/daos/movie_dao.dart';
...
class MovieDaoImplMock extends MovieDao{
  ...
  List<MovieVO> getNowPlayingMovies() {
    if((getMockMoviesForTest().isNotEmpty)){
      return getMockMoviesForTest()
          .where((element)=>element.isNowPlaying==true ?? false)
          .toList();
    }else{
      return [];
    }
  ...
  List<MovieVO> getPopularMovies() {
    if (getMockMoviesForTest() != null && (getMockMoviesForTest().isNotEmpty ?? false)) {
      return getMockMoviesForTest()
